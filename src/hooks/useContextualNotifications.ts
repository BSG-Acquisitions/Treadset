...
export const useContextualNotifications = () => {
  ...
  useQuery({
    queryKey: ['incomplete-manifests-check', orgId],
    queryFn: async () => {
      ...
      // Single bulk query for existing notifications
      ...
      const notifiedIds = new Set((existingNotifs || []).map(n => n.related_id));

      for (const manifest of manifests) {
        if (notifiedIds.has(manifest.id)) continue;
        createNotification({
  ...
  useQuery({
    queryKey: ['missing-client-data-check', orgId],
    queryFn: async () => {
      const { data: userData } = await supabase
        .from('users')
        .select('id')
      ...
      for (const client of clients.slice(0, 5)) {
        if (notifiedIds.has(client.id)) continue;
  ...
  useQuery({
    queryKey: ['unassigned-pickups-check', orgId],
    queryFn: async () => {
      const { data: userData } = await supabase
        .from('users')
        .select('id')
      ...
      for (const pickup of pickups.slice(0, 10)) {
        if (notifiedIds.has(pickup.id)) continue;
        createNotification({
          user_id: internalUserId,
          organization_id: orgId!,
