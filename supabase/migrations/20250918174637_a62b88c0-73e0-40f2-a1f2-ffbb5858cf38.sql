-- Complete remaining security and performance fixes
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_locations_client_active ON public.locations(client_id, is_active);
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_audit_events_org_date ON public.audit_events(organization_id, created_at DESC);
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_manifests_org_status_date ON public.manifests(organization_id, status, created_at DESC);

-- 6. Optimize pricing queries
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_pricing_tiers_org_active ON public.pricing_tiers(organization_id) WHERE organization_id IS NOT NULL;

-- 7. Add missing foreign key constraints for data integrity
DO $$
BEGIN
  -- Add foreign key for dropoffs -> dropoff_customers if it doesn't exist
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.table_constraints 
    WHERE constraint_name = 'fk_dropoffs_dropoff_customer'
    AND table_name = 'dropoffs'
  ) THEN
  ...
  -- Add foreign key for assignments -> users (driver_id) if it doesn't exist  
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.table_constraints 
    WHERE constraint_name = 'fk_assignments_driver'
    AND table_name = 'assignments'
  ) THEN
    ALTER TABLE public.assignments 
    ADD CONSTRAINT fk_assignments_driver 
