-- =============================================
-- Supabase Performance Fixes (RLS auth wrappers + duplicate indexes)
-- Corrected quoting inside DO blocks
-- =============================================
...
BEGIN
  ...
  IF EXISTS (
    ...
    EXECUTE 'ALTER POLICY "Allow viewing organizations when authenticated or auth disabled" 
  ...
  IF EXISTS (
    ...
    EXECUTE 'DROP POLICY "Allow managing organizations when authenticated or auth disable" ON public.organizations;';

    -- INSERT
    EXECUTE 'CREATE POLICY "Allow inserting organizations when authenticated or auth disabled" 
             ON public.organizations 
             FOR INSERT 
             USING (((select auth.uid()) IS NULL) OR ((select auth.uid()) IS NOT NULL)) 
    ...
    -- UPDATE
    EXECUTE 'CREATE POLICY "Allow updating organizations when authenticated or auth disabled" 
             ON public.organizations 
             FOR UPDATE 
             USING (((select auth.uid()) IS NULL) OR ((select auth.uid()) IS NOT NULL)) 
             WITH CHECK (((select auth.uid()) IS NULL) OR ((select auth.uid()) IS NOT NULL));';
    ...
    -- DELETE
    EXECUTE 'CREATE POLICY "Allow deleting organizations when authenticated or auth disabled" 
             ON public.organizations 
             FOR DELETE 
             USING (((select auth.uid()) IS NULL) OR ((select auth.uid()) IS NOT NULL));';
  END IF;
...
BEGIN
  IF EXISTS (
    ...
    EXECUTE 'ALTER POLICY "Allow user operations when authenticated or auth disabled" 
             ON public.users 
             USING (((select auth.uid()) IS NULL) OR ((select auth.uid()) IS NOT NULL)) 
             WITH CHECK (((select auth.uid()) IS NULL) OR ((select auth.uid()) IS NOT NULL));';
  END IF;
...
BEGIN
  IF EXISTS (
    ...
    EXECUTE 'ALTER POLICY "Allow access to user organization roles" 
             ON public.user_organization_roles 
             USING (((select auth.uid()) IS NULL) OR (EXISTS (SELECT 1 FROM public.users WHERE users.auth_user_id = (select auth.uid()))));';
  END IF;
END $$;
...
BEGIN
  IF EXISTS (
    ...
    EXECUTE 'ALTER POLICY "Users can access manifests in their organizations" 
             ON public.manifests 
             USING (((select auth.uid()) IS NULL) OR (organization_id IN (SELECT uo.organization_id FROM public.user_organization_roles uo JOIN public.users u ON uo.user_id = u.id WHERE u.auth_user_id = (select auth.uid()))));';
  END IF;
END $$;
...
BEGIN
  IF EXISTS (
    ...
    EXECUTE 'ALTER POLICY "Users can access workflow data in their organizations" 
...
BEGIN
  IF EXISTS (
    ...
    EXECUTE 'ALTER POLICY "Users can view their own preferences" 
  ...
  IF EXISTS (
    ...
    EXECUTE 'ALTER POLICY "Users can create their own preferences" 
             ON public.user_preferences 
  ...
  IF EXISTS (
    ...
    EXECUTE 'ALTER POLICY "Users can update their own preferences" 
             ON public.user_preferences 
             USING (((select auth.uid()) = user_id));';
  END IF;
END $$;
